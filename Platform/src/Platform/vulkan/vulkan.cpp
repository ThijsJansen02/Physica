...
namespace PH::Vulkan {
	...
	VkSurfaceFormatKHR chooseSurfaceFormat(Array<VkSurfaceFormatKHR> availableformats) {
		for (const auto& format : availableformats) {
			...
			else if (format.format == VK_FORMAT_B8G8R8A8_SRGB && format.colorSpace == VK_COLORSPACE_SRGB_NONLINEAR_KHR) {
				return format;
			}
		}
	...
	bool isDeviceSuitable(VkPhysicalDevice device, VkSurfaceKHR surface, Array<const char*> deviceextensions) {
		...
		return features.samplerAnisotropy && swapchainadequate && extensionssupported && queuefamiliessupported;
	}
