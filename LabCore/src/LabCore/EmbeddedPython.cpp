...
namespace PH::LabCore {
	...
	void initPython(const char* pythonhome) {
		...
		PyConfig_SetString(&config, &config.program_name, L"LabCore");

		config.module_search_paths_set = 1;

		Engine::INFO << "Python home set to: " << pythonhome << "\n";
		...
		try {
			...
			Engine::ERR << "Python Error: " << e.what() << "\n";
		}
		catch (std::runtime_error& e) {
			Engine::ERR << "Runtime Error: " << e.what() << "\n";
		}
		catch (std::exception& e) {
			Engine::ERR << "Exception: " << e.what() << "\n";
		}
